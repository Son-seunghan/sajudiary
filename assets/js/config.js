...
window.SAJULOG_CONFIG = {
  ...
  MASTER_KAKAO_IDS: [
    'kakao_4876030261',                 // 손승한 (본인) — 실제 카카오 ID (2026-08-01, 모든 기기 공통)
    'kakao_fb_1779194706531_v4iqwre9',  // 손승한 — 구 폴백 ID (레거시, 유지해도 무해)
  ],
  ...
  // ─────────────────────────────────────────
  // 3-A. 카카오페이 직접 결제 (독립형 가맹점)
  // ─────────────────────────────────────────
  // 가맹 승인: 2026-07-17 · CID: CT11549602 (실제 CID는 서버 환경변수 KAKAOPAY_CID 사용)
  // 서버리스 함수: /api/kakaopay-ready, /api/kakaopay-approve
  // Secret Key는 Vercel 환경변수 KAKAOPAY_SECRET_KEY에만 저장 (코드·깃 노출 금지)
  KAKAOPAY: {
    ENABLED: true   // false → 결제 페이지에서 카카오페이 버튼 숨김
  },
